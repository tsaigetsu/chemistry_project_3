...
import alcoBg from '../assets/images/alco-bg.png';  // Импортируем картинки для фона
import pyroBg from '../assets/images/pyro-bg.png';  // Импортируем картинки для фона

const History = ({ historyData, isVisible, onToggle, t, theme }) => {
  // Выбираем фоновую картинку в зависимости от темы
  const backgroundImage = theme === "alcohol" ? `url(${alcoBg})` : `url(${pyroBg})`;
    ...
    >
      ...
      <div className="section-wrap">
        <button className="toggle-button" onClick={onToggle}>
          {isVisible ? '▲' : '▼'} {/* Стрелка меняет направление */}
        </button>

        {isVisible && (
          <div className="history-content">
            <h2 className={`history-title ${theme}`}>{t.history}</h2> {/* Заголовок секции зависит от темы и языка */}
            <div className="history-details">
              {historyData.map((item, index) => (
                <p className="history-p" key={index}>{item.history}</p> // Отображаем историю как текст
              ))}
            </div>
          </div>
        )}
